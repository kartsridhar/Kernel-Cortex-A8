...
void putsShell( char* x, int n ) {
  for( int i = 0; i < n; i++ ) {
    PL011_putc( UART1, x[ i ], true );
  }
}

void getsShell( char* x, int n ) {
  for( int i = 0; i < n; i++ ) {
...
void main_shell() {
  ...
  while( 1 ) {
    putsShell( "shell$ ", 7 ); getsShell( x, 1024 ); p = strtok( x, " " );

    if     ( 0 == strcmp( p, "execute"   ) ) {
      pid_t pid = fork();

      if( 0 == pid ) {
        exec( loadShell( strtok( NULL, " " ) ) );
      }
